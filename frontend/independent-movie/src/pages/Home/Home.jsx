...
export default function Home() {
  ...
  // 슬라이드 인덱스 (가짜 카드(복제) 때문에 LOOP_COUNT부터 시작)
  const [premiumIndex, setPremiumIndex] = useState(LOOP_COUNT);
  const [movieIndex, setMovieIndex] = useState(LOOP_COUNT);
  ...
  /* ================= 무한 슬라이드 배열(앞/뒤 복제 붙이기) ================= */
  const premiumLoop =
    basePremium.length === LIMIT_COUNT
      ? [
  ...
  /* ================= 순간이동 처리(경계에서 자연스럽게 이어지게) ================= */
  useEffect(() => {
    if (basePremium.length !== LIMIT_COUNT) return;
  ...
  }, [premiumIndex, basePremium.length]);

  useEffect(() => {
    if (baseMovies.length !== LIMIT_COUNT) return;
